  ...
  <div class="quiz-wrapper">
    ...
    <template v-else>
      ...
      <div class="quiz-body" v-else>
        Siz {{ QuizList.length }} ta savoldan {{ isCorrectAnswer }} tasiga javob
        berdingiz!
        <button class="btn" @click="fetchList">Qayta ishlash</button>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions("test", ["fetchListType", "checkFinished"]),
    async finished() {
      let response = [];
    ...
    async fetchList() {
      try {
        ...
        this.ActiveQuiz = 0;
        this.loading = true;
        let res = await this.fetchListType({ type: 1 });
